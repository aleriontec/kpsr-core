...
kpsr::mem::BasicScheduler::ScheduledThread::ScheduledThread(int after, bool repeat,  std::function<void ()> task)
    : _after(after)
    , _repeat(repeat)
    , _isRunning(false)
    , _task(task)
...
void kpsr::mem::BasicScheduler::startScheduledTask(const std::string & name, int after, bool repeat,  std::function<void ()> task) {
